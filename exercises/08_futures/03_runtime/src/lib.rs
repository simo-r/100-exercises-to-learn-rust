    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let first_handle = tokio::spawn(async_reply(first, Arc::clone(&reply)));
    let second_handle = tokio::spawn(async_reply(second, reply));
    tokio::join!(first_handle, second_handle);
}

async fn async_reply<T>(listener: TcpListener, reply: Arc<T>)
where
    // `T` cannot be cloned. How do you share it between the two server tasks?
    T: Display + Send + Sync + 'static,
{
